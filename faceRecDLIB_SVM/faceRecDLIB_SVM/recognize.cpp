		...
		while (cv::waitKey(30) != 27)
		{
			...
			if (!shapes.empty()) {
				...
				for (int i = 0; i < 68; i++) {
					...
					testData[0][i * 2] = (shapes[0].part(i).x() - faces[0].left()) / fRatio;
					testData[0][i * 2+1] = (shapes[0].part(i).y() - faces[0].top()) / fRatio;
				...
				if (svm->predict(query) == 170) {
					cv::putText(temp, "Peace", cv::Point(20, 60), 3, 2, cvScalar(0, 0, 255));
					cout << "Peace" << endl;
				}
				if (svm->predict(query) == 300) {
					cv::putText(temp, "Annoyed", cv::Point(20, 60), 3, 2, cvScalar(0, 0, 255));
					cout << "Annoyed" << endl;
				}
